...
import { parentChatCredentials, parentchatMessage, nannychatMessage } from '@testdata/parachute/Nanny/parachute-nanny-test-data';
...
const selectUser = `//span[contains(text(),'Regina shekar')]`
const parentTypeMessage = `//*[@contenteditable='true']`
...
export async function parentChatlogInSuccessfully() {
    await waitForElementToBeVisible(emailAddress)
    await fill(emailAddress, parentChatCredentials.username);
    await fill(password(), parentChatCredentials.password);
...
export async function parentVerifyNannySendedMessage(){
...
export async function fillmessage(){
    ...
    await fill(parentTypeMessage, parentchatMessage.parentMessage)
}
